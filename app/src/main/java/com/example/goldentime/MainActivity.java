...
public class MainActivity extends AppCompatActivity
 ...
 //   public native void ConvertRGBtoGray(long matAddrInput, long matAddrResult);
   ...
   /* public int timer(){
        int ret = detect(cascadeClassifier_face, matInput.getNativeObjAddr(),
                matResult.getNativeObjAddr());
        int alarm;
        if(ret==0){
            for(int human_count=0;human_count<10;human_count++){
                if(ret!=0){
                    human_count=0;
                }
            }
            alarm=1;
            Log.d(TAG, "얼굴이 인식되지 않습니다"+alarm);
            return alarm;
        }
        return ret;
    }*/
